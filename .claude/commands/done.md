...
**메모리 기록**: 🔔 출력 시 **CC auto-memory의 `pending_founder_actions.md`**(세션 자동 로드본, `/memory`로 관리 — 프로젝트 로컬 `memory/` 아님)에 아래 형식으로 행 추가:
```
| [오늘날짜] | [액션 내용] | [커밋 SHA] | ⏳ 대기 중 |
```
창업자가 완료 확인 시 해당 행을 `✅ 완료`로 업데이트. 자율 위임 분류(AUTO/HANDOFF/WAIT) 규칙: `.claude/rules/autonomy.md`.
