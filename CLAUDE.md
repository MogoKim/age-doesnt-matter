...
3. **커밋+푸시는 /done 스킬로 자동화**: 작업 완료 시 `/done`을 호출하면 Gate 1 검증(tsc + 크론 + 빌드 + AI 아키텍처) → 전부 PASS 시 **자동으로 커밋+푸시** (묻지 않음). 실패 시 Claude 대화창 + Slack #qa 알림.
   - **Gate 1 PASS → 자동 커밋+푸시** (Gate 2는 Vercel 배포 후 GitHub Actions에서 자동 실행)
   - **커밋+푸시 불필요 상황**: 코드 읽기·분석·조사만 수행 / 플랜(`.claude/plans/`)·메모리(`.claude/memory/`) 파일 — 로컬 전용 / 임시 디버그 스크립트 실행 후 버리는 경우
   - 예외: 창업자가 "커밋만" 명시 → push 생략 / "지금 바로 push해" 명시 → Gate 1 스킵 가능 (대화창에 명시)
