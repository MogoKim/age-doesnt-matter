...
### 제품/브랜드 규칙
8. **"시니어" 용어 절대 금지**: "시니어", "액티브 시니어" 사용 금지. 대체: "우리 또래", "50대 60대", "인생 2막" 등 자연스러운 표현.
9. **네비게이션**: 하단 탭바 X → 상단 아이콘 메뉴 행 + 플로팅 FAB("✏️ 글쓰기")
10. **카카오 정보 수집**: 필수 최소화 (providerId/닉네임/프로필만 자동), 나머지 선택 동의. 가입 허들 낮추기.
11. **어드민 계정**: 창업자가 자유롭게 생성/닉네임 설정 가능해야 함.
12. **운영 채널**: Telegram → Slack 전면 전환 완료. 13개 채널 운영 중.
13. **디자인 워크플로우**: 코드가 디자인 토큰 원본, Figma는 시각적 참조. 개발 먼저 → Figma 반영도 OK.
