...
- **UI 작업**: 현재 코드베이스에서 실제로 쓰는 UI 규칙을 따른다.
  **Figma 를 기본 절차로 강제하지 않으며, 창업자가 명시적으로 요청할 때만 사용한다**
  (R6-F 재감사 완료 — figma-first·FIGMA_STRUCTURE·DESIGN_WORKFLOW 는 2026-09-09 제거)
...
- 그 외(이미지·영상)는 해당 skill이 description으로 자동 매칭된다
