...
- '아이/딸/아들'이 있어도 성인 자녀·손주·조카·지인 이야기면 정상 PASS다(other_person_story).
  **[또래 문맥 전이 — 예외(창업자 확정 2026-07-17)]** 단, 그 타인이 화자의 '친구/또래'이고 내용이
  임신 준비/난임/영유아·미취학 자녀(6살 등) 자랑이면 화자도 같은 세대라는 신호다 — 60대는 "친구가
  6살 애 자랑해서 곤란하다"는 고민을 하지 않는다. 이 경우 other_person_story로 PASS시키지 말고
  young_self 계열 REJECT 후보로 판정하라. 자녀·손주·며느리·사위 세대 이야기는 기존처럼 정상이다.
...
- **[현재 양육 우선 규칙 — 위 가족 갈등 PASS보다 항상 우선한다(창업자 확정 2026-07-16)]**
  ...
  **[적용 주체 한정 — 2026-08-14]** 이 규칙은 **화자 본인이 양육 중일 때만** 쓴다.
  시누·올케·동서·친구·이웃 등 **타인의 자녀·조카·손주** 이야기는 화자의 현재 양육이 아니다 —
  parenting_current를 붙이지 마라. 예: "시조카 돌잔치 가야 하나요", "손주 봐달라는데" → 관계글로 PASS 후보.
  다만 본문에 화자 본인의 영유아~중등 자녀 신호가 별도로 있으면 그때는 위 원칙대로 REJECT다.
  **[고등 자녀 예외 — 창업자 확정 2026-07-19]** 이 규칙의 차단 범위는 영유아~중등까지다.
  고등학생 자녀를 둔 엄마/학부모 시점 글은 화자가 40대 중반~50대 = 우나어 타깃일 수 있으므로
  '고딩/고등학교/여고딩' 단어만으로 parenting_current REJECT하지 마라(정답 표본: "여고딩끼리 가평
  빠지 간다는데 보내주시겠어요?" — 고딩 딸 외출 걱정하는 엄마 = 타깃 정중앙, PASS).
  단 아래는 예외가 아니다: ①학생 본인 1인칭 발화(교복을 '내가' 버리려는데 등) → young_self REJECT
  ②"30,40대" 자기 호출 동반 → young_self REJECT ③초등~중등 형제 병행 양육 동반 → parenting_current REJECT
  ④신혼·결혼 초기·젊은 맘카페 문맥 동반 → 기존대로 REJECT.
  **[예외의 경계 명확화 — 2026-08-14 차단 로그 감사]** 위 예외가 실제로는 잘 지켜지지 않아
  "고2 아이 1박2일 여행 보내도 될까요"류(미성년 고등 자녀의 외출·독립·간섭 고민)가 REJECT됐다.
  이건 정답 표본("여고딩끼리 가평 빠지")과 같은 유형이므로 **PASS다.**
  · 고등 자녀의 **외출·여행·친구관계·독립·간섭 고민** → PASS (자녀를 놓아줄지 고민하는 중장년 부모 시점)
  · 고등 자녀의 **입시·학원·과외·성적 관리 현재형** → parenting_current REJECT 유지 (교육 관리 주체)
  경계가 애매하면 REJECT가 아니라 NEEDS_REVIEW로 두어라 — 좋은 관계글을 잃는 손해가 더 크다.
...
- **[연령대 자기 호출 — 창업자 확정 2026-07-17]** 작성자가 자기 집단을 "30,40대 분들"/"3040"/
  "30대 맘들"/"30대 40대 분들"로 부르면 우나어 타깃(40대 중반~60대) 이탈 신호다 —
  애매한 NEEDS_REVIEW가 아니라 **young_self 계열 REJECT 후보**로 판정하라.
  단, "30대 자녀"/"40대 아들"/"30대 후배"처럼 타인을 지칭하는 경우와 "50,60대 분들" 자기 호출은 제외.
- **[결혼 연차 산술 — 낙관 추정 금지]** "결혼 N년차(15년차 이하)" 자기언급 + 미성년/사춘기 자녀 +
  중장년 단서(성인자녀/손주/갱년기/노후/결혼 20~30년차) 부재면 화자는 30대 후반~40대 초반 가능성이
  높다 — 근거 없이 40대 중반 이상으로 추정해 PASS시키지 마라(정답 표본: "결혼 13년차 외동자녀
  맞벌이 엄마" + 제목 "30,40대 분들" = REJECT). "30,40대" 자기 호출이 함께 있으면 REJECT다.
