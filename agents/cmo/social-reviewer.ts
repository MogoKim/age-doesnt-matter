...
/**
 * CMO Social Reviewer — 3일 주기 실행 (월+목 10:00 KST)
 *
 * 지난 3일 실험 데이터를 분석하여:
 * 1. 통제군 vs 실험군 성과 비교
 * 2. 콘텐츠 유형/톤/시간/페르소나별 성과 랭킹
 * 3. 카드뉴스 v2: 카테고리별 + 슬라이드 수별 분석
 * 4. 우승자 판별 + 인사이트 도출
 * 5. SocialExperiment.learnings 기록
 */
...
async function main() {
  ...
  const reviewPeriodMs = 3 * 24 * 60 * 60 * 1000  // 3일 주기
  const periodStart = new Date(Date.now() - reviewPeriodMs)

  // 1. 지난 3일 게시물 조회
  const posts = await prisma.socialPost.findMany({
    where: { status: 'POSTED', postedAt: { gte: periodStart } },
  ...
  // 4. AI 인사이트 도출
  const summaryData = `
지난 3일 SNS 성과 분석 (총 ${posts.length}개 게시물):
...
`
  ...
  await notifySlack({
    level: 'info',
    agent: 'CMO_SOCIAL',
    title: `SNS 리뷰 (3일) — ${posts.length}개 분석 완료`,
    body: [
      `*분석 기간*: ${periodStart.toLocaleDateString('ko-KR')} ~ ${new Date().toLocaleDateString('ko-KR')}`,
