...
const HANDLERS: Record<string, () => Promise<void>> = {
  ...
  // LOCAL ONLY — 공개 시드 글 3건 정합성(2026-09-14). 크론에 연결하지 않는다.
  // 기본 dry-run. 실제 변경은 `-- --execute --confirm=<토큰>` 이 둘 다 있어야 한다.
  'coo:seed-post-integrity': () => import('../coo/seed-post-integrity.js').then(async (m) => { await m.main(process.argv.slice(4)) }),
