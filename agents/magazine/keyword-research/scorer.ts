...
export function inferCluster(keyword: string): ClusterId | 'uncategorized' {
  ...
  // ⑤ 갱년기건강 (넓음 — 뒤로). 일반 건강 토큰(건강/질병/우울/폐경 등) 보강 —
  //    "60대 여성 건강"처럼 부모(패션생활)를 잘못 상속하던 케이스를 자기 내용으로 재분류.
  //    부부성건강·외로움관계·일자리·돈연금이 위에 있어 "갱년기 질건조"→부부성건강, "혼자/빈둥지"→외로움관계는 그대로 유지.
  if (/갱년기|건강|질병|질환|우울|폐경|면역|통증|치매|혈관|혈압|당뇨|관절|무릎|영양제|수면|불면|다이어트|호르몬|골다공증|증상|병원/.test(keyword)) {
    return '갱년기건강'
  }
  // ⑥ 패션생활
