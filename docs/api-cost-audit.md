...
> ⚠️ **ARCHIVED (2026-09-09)** — 2026-05-11 시점 기록이다. 그 뒤 R4 에서 자동화 대부분을 제거해
> (핸들러 93 → 6) API·이미지 생성 비용 구조가 크게 달라졌다. **현재 비용으로 인용하지 마라.**
> 재산정은 청구서 기준으로 별도 수행한다.
...
| 항목 | 5월 실측 | 성격 | 비고 |
|------|---------|------|------|
| **Claude Max 구독** | **~$110/월** | 개발자 도구 | $100 base + VAT. 5x 사용량 플랜 |
| Anthropic API (에이전트) | ~$55/월 | 서비스 인프라 | $11 자동충전 × ~5회/월 |
| Vercel Pro | ~$22/월 | 서비스 인프라 | Base $20 + 사용 $1.92 |
| Google Gemini | ~$8/월 | 개발/도구 | ₩11,000 구독 |
| OpenAI DALL-E | ~$1.5/월 | 서비스 인프라 | CMO 카드뉴스 이미지 |
| Supabase / Upstash / Resend / Cloudflare | $0 | 서비스 인프라 | 무료 플랜 |
| 도메인 | ~$1.5/월 | 기타 | 연간 환산 |
| **실제 총합** | **~$198/월** | — | 이전 추정 $87 = Claude Max 누락됐었음 |

> ⚠️ Claude Max 구독($110)은 개발자 도구 비용. 개발 활동 없는 기간 Pro($20)로 다운그레이드 시 $90 즉시 절감 가능.  
> ✅ Vercel 최적화 효과: 4월 ~$280 → 5월 ~$22. 연간 환산 $3,096 절감.
