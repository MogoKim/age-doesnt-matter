...
- 게이트: cap=8 댓글 wave 검증
- 현재 결정: 20:30 KST 저녁 피크 검증과 2026-05-31 오전 24h 검증 전까지 새 구현은 시작하지 않는다.
- 지금 가능: read-only 진단, 프롬프트 준비, 백로그 정리
- 지금 금지: cap 변경, persona pool 확장, wave 로직 변경, generator 변경, PostCTA 구현, metadata 수정
- 운영 메모: 2026-05-30 저녁 스크래퍼 입력 소스에 오늘의 유머, 네이트판, 네이버 카페, 82cook 링크가 추가됨. 신규 소스 품질/댓글 품질은 SHEET 댓글 AI티 개선과 함께 확인한다.
