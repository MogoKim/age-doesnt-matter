...
> ⚠️ **ARCHIVED (2026-09-09)** — 여기 설명하는 카페 크롤·스크래퍼·페르소나 시스템은 **R4 에서 전부 제거됐다.**
> 2026-05-12 시점의 실측 기록으로만 읽는다. 현재 동작을 설명하지 않으며, 여기 적힌 경로·에이전트는 존재하지 않는다.
> 현재 자동화 목록은 `docs/SERVICE_ARCHITECTURE.md` §5.2 를 본다.
