...
> ⚠️ **실측 2026-09-16** — 이 문서가 가리키는 구현 경로 **4개가 더 이상 존재하지 않는다** (그중 3개는 `agents/` 하위 — 2026-09 네이버 카페 폐기·R4 정리로 삭제됨).
...
| 2026-05-27 | `compute-usable-count.ts` 공통 helper로 usable 계산 기준 통일 — wave-processor.ts 로컬 `removeEmoji`·`WP_AI_REJECT_RE`·`computeUsableCount` 제거 후 shared import. content-curator의 느슨한 usable 기준(10자 이상만)을 wave-processor와 동일 기준(emoji제거+AI패턴필터+dedup)으로 통일 | content-curator usable≥5 통과 글이 wave-processor에서 재계산 시 usable<5로 판정 → BLOCK2 재발 가능성 구조적 제거 |
| 2026-05-27 | `compute-usable-count.ts`에서 `removeEmoji` export 추가 + `wave-processor.ts` import 보완 (ffbfd93) | 9a879a5에서 로컬 함수 제거 시 `getV2SourceCandidates`(636라인) 내 단독 `removeEmoji` 호출 누락 → ReferenceError로 wave1~4 전부 실패 |
| 2026-05-27 | 2차 구현 — `syncPopularPosts` Case A/B `crawlPost(..., true)` 변경 + Case A 덮어쓰기 방지(usable≥5 조건부 update) + Case B topComments/commentCrawled create 추가. `popular-curator.ts`: 로컬 computeUsableCount 제거→공통 helper, rawCandidates WHERE `commentCrawled: true` + usable≥5 필터(`usableCandidates`) + WAVE_SKIP_USABLE_ZERO BotLog + details 상세화 | popular 글 구조적 댓글 0건 문제 해결 — syncPopularPosts가 topComments를 수집하고, popular-curator는 usable≥5 글만 발행 → wave1~4 full run 보장 |
| 2026-05-30 | `BOT_DAILY_COMMENT_CAP` 3→8 상향 + 공통 상수로 통합 — legacy/v2 각자 로컬 const 2개를 모듈 상단 단일 상수로 일원화 | 48h 실측: STORY+LIFE2 445슬롯/일 수요 vs 67봇×3=201 공급(45%), 전체 충족률 40% → cap=8(536슬롯)으로 수요 커버. bot_cap 600건/48h 해소 목적 |
