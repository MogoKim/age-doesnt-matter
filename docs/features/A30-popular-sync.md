...
| 2026-06-07 | `curator-shared.ts` `guessDesire()` substring 첫-매치 → **가중 스코어링** 전환(LIFE2 강신호 +3, STORY_GUARD −3, 동점 STORY 우선, LIFE2 내부 동점=선언순서). `DESIRE_KEYWORDS` LIFE2 강신호 추가(폭락·재산분배·상속·증여·퇴직연금·임피·dc전환·집값·매매 등) + HOUSING `집` 약신호 제거. `STORY_GUARD_KEYWORDS`(집정리/윗집/발망치/고장/병간호 등) + `isStoryGuarded()` 신규 export. `image-router.ts` `MONEY_CATS`에서 JOB 제거 + `resolveTab` 직전 제목 guard 후처리(psych 경로 LIFE2 강등) | 창업자 어드민 STORY↔LIFE2 교정 16건 학습 — '집정리/윗집'이 `집`으로 LIFE2 과잉분류, '폭락장/재산분배/퇴직연금'이 LIFE2 미분류되던 문제. 회귀검증 21/21 통과, 분포 diff로 `집` 누수 제거 확인 |
