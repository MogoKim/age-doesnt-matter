...
| 헤드라인 | "나만 이런 게 아니었네?" |
| 서브 | "우리끼리 편하게 수다 떨어봐요" |
| CTA | "카카오 한 번 클릭으로 가입" (iOS는 인앱 포함 "카카오로 1초 가입" 계열 OAuth 직행, Android 인앱은 "카카오로 가입하기" 문구 + 기존 외부 브라우저 유도 경로) |
...
- 2026-08-10: iOS 인앱 SignupPromptBanner CTA의 주소복사/Safari 붙여넣기 안내를 제거하고 기존 카카오 OAuth 직행 정책으로 복구했다.
- 2026-08-10: Android 인앱 SignupPromptBanner CTA에서 "브라우저/카카오 밖" 같은 기술 경로 문구를 제거하고 가입 중심 문구로 정리했다.
...
| 2026-08-11 | **인앱 한정** 60초 백스톱 제거 — 인앱은 정독 85% 도달만 트리거. 비인앱(Android 외부 브라우저 A/B 대상·iOS Safari/Chrome·Desktop)은 기존 백스톱 그대로 | 인앱은 백스톱이 정독보다 먼저 걸려 글을 42%밖에 안 읽은 시점에 배너를 만났고(데스크탑 86%), 2.7초 만에 ✕로 닫혔다(3초 내 63.6%). 구조적 방해는 PR #318에서 제거했으므로 남은 원인이 타이밍이다. **문구·디자인·클릭 경로·횟수 정책 무변경** (F20 §8) |
