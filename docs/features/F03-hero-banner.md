...
| 2026-04-27 | 홈 page.tsx Promise.all 제거 → 섹션별 Suspense 스트리밍, HeroSlider 즉시 전송 | LCP Load Delay 3,121ms → ~400ms 목표 |
| 2026-05-02 | HeroSlider Suspense 래핑 + auth() 페이지 상단 제거 → PersonalGreeting/SignupCard async wrapper 분리 | cold-start 시 auth() 블로킹으로 HeroSlider 포함 전 HTML이 지연됨 → TTFB 개선 |
| 2026-05-10 | HomeFaqSection 컴포넌트 추가 (FAQPage Schema + FAQ UI 5개), SignupCard 위에 배치 | GEO(AI 검색) 인용 최적화 — 홈페이지 FAQPage JSON-LD Schema 없음 |
