...
| 2026-06-02 | force-dynamic 제거 + auth() wrapper 4개 → useSession() 전환 (PersonalGreeting/HomeFaqSection/SignupCard), MyActivity 임시 비노출 | / CDN HIT 활성화 (Static 전환) |
...
| 2026-05-17 | 뜨는이야기 쿼터제 개편 — getTrendingQuotaPosts 신규(STORY 6+LIFE2 2+HUMOR 2, trendingScore 재정렬), 사는이야기 raw fetch 10→12, PostSummary에 trendingScore 필드 추가 | LIFE2(2막준비) 글이 STORY에 밀려 홈 노출 0인 문제 해결, 뜨는이야기 5→10개 확대 |
| 2026-05-18 | trendingScore 알고리즘 개편 — noonAge(정오 기준) → postAge(글 작성 시점 기준) 24시간 롤링. posts.home.ts 3단계→2단계 폴백(24h+engagement → 7일 fallback). calculateTrendingScore에 createdAt 파라미터 추가 | 업로드 타이밍(11:55 vs 12:05)에 따른 점수 유불리 제거, 실시간성 강화 |
