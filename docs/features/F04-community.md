...
| 2026-05-15 | 글쓰기 모바일 UX 전수검사 — [C1] 이미지 전용 게시글 클라-서버 검증 불일치 수정(posts.ts hasMedia), [C2] span 미허용으로 글자크기 서식 소실 수정(sanitize.ts), [H2] handleCancel hardlink 전환, [H3] 동영상 50MB 통일, [H4] draft 저장실패 toast, [M1/M2] 터치타겟 44→52px, [M3] CASCADE padding, [M5] delete 피드백, [M6] videoEl DOM 추가, [L1] aria-label | 전수검사 3라운드 — 클라-서버 불일치 버그 2개(CRITICAL) + 시니어 친화 터치타겟 미달 일괄 수정 |
...
| 2026-06-03 | PostCard boardType별 카테고리 칩 색상 분리 — getCategoryChipClass() 적용 (showBoardBadge 양쪽) | 게시판별 색상 시각화로 콘텐츠 구분 개선 |
