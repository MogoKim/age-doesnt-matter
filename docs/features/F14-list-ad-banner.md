...
| 2026-06-05 | 슬롯 노출을 '목록 상단 띠' 하나로 축소(HOME_INLINE도 미연결 확인) | 실제 동작하는 슬롯만 노출 |
| 2026-06-05 | 이미지 업로드를 presign 직접 PUT → 서버 경유(`/api/admin/uploads/banner`)로 변경 | vercel.app 등에서 R2 직접 PUT CORS 차단 → 서버 경유로 도메인 무관 동작 |
| 2026-06-05 | 노출 위치 칩 UX 수정: 전체공통(빈값)일 때 6개 칩 모두 ON 표시, 칩 끄면 해당 페이지 제외, 1개 남고 끄면 자동 전체 복귀 | 6개 전부 누르면 칩이 다 풀려(선택 안 됨처럼) 보이던 혼란 해소 — 저장·노출 필터는 무변경(빈값=전체공통 호환) |
| 2026-06-06 | 광고 슬롯 탭에서 "최상단 띠 배너" 탭 버튼 누락 수정 (TAB_ITEMS에 top-promo 추가) | BannerManager(3탭)와 불일치 — 광고슬롯 진입 시 최상단띠 탭이 사라지던 버그. 탭 네비 중복 정의 잔재 |
