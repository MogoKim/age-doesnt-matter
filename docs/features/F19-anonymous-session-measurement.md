...
>
> **§7 추천안 A가 구현됐다** (2026-08-07, 후속 구현 PR). 기준 자체는 이 문서가 계속 정본이다.
> 코드 기준: `origin/main` `b7cb9aa3` / 실측 기준: 2026-08-07 10:35 KST (문제 진단 시점 — 아래 수치는 **구현 전** 값)
...
## 12. 후속 구현 PR 범위 — ✅ 적용됨 (2026-08-07)

**구현 결과 요약** (상세는 PR 본문·코드 주석)

- `src/lib/anon-cid.ts` 신규 — 식별자 생성·검증·해석을 **한 파일에 모아 클라/서버 단일 진실**로 뒀다. 브라우저 전용 함수는 `typeof window` 가드.
- `trackEvent`가 **중앙에서** `properties.anon_cid`를 붙인다. 호출부 API·엔드포인트 무변경, 추가 네트워크 요청 0.
- `/api/events`는 `anon_cid → _anon_sid → 신규 UUID` 순으로 sessionId를 정한다. **결정된 값이 `_anon_sid` 쿠키로도 내려가 localStorage와 쿠키가 한 값으로 수렴**한다.
- 어드민 집계 4종에 우선순위(`userId → anon_cid → sessionId`) 적용. `getDailyTrend`는 의도적 예외(코드 주석에 사유).
- **읽기가 막힌 스토리지에서는 새로 만들지 않는다** — 만들면 페이지 로드마다 값이 달라져 쿠키 fallback보다 나빠지기 때문(테스트로 고정).

**남은 한계**: Safari ITP(§7)는 그대로다. `web_vital` 이벤트는 `trackEvent`를 우회해(`WebVitalsReporter.tsx:46`) `anon_cid`가 없다 — `page_view` 기반 지표(UV·리텐션·실험 분모)에는 영향 없어 이번 범위에서 제외했다.
...
| 2026-08-07 | **§7 추천안 A 구현 적용** — `anon-cid.ts` 신규 + `trackEvent` 중앙 동봉 + `/api/events` 우선순위 + 어드민 집계 4종. §12에 결과·한계 기록 | 기준 확정 후 구현. 첫 방문 동시 이벤트 파편화(비회원 재방문 52.9% 유실) 해소 |
