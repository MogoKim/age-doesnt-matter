...
> 🔴 **2026-09-22 정책 변경 — 우나어 가입 유도 중단 (Project BRIDGE)**
> Project BRIDGE(소란소란 이주 유도)와 메시지가 충돌해 우나어 가입 유도를 중단.
> | 채널 | 이전 | 현재 |
> |---|---|---|
> | 글 정독 배너(F01) | 정독 85퍼센트/60초 백스톱 → 5초 자동 가입 | **마운트 해제(중단)** |
> | 홈 가입 카드(F03) | 비회원에게 카카오 가입 카드 | **제거** |
> | 글 하단 CTA(F13) 비회원 | "1초 만에 가입하기" | **렌더 없음** |
> | 글 하단 CTA(F13) 로그인 | 앱 설치 유도 | **유지** |
> | 로그인 필요 모달 | 댓글·공감 시 로그인 요구 | **유지**(기능상 필수) |
>
> ⚠️ 함께 중단된 것: 이 배너의 CTA가 담당하던 **인앱 브라우저 → 외부 브라우저 유도**
> (`inapp-redirect.ts` 경로). 코드는 남아 있으나 호출 지점이 없다.
> ⚠️ 계측: `signup_banner_*`(F01)와 `cta_type='signup'`(F13) 이벤트가 더 이상 발생하지 않는다.
> **과거 데이터와 이어 붙여 추세로 읽지 말 것** — 0은 성과 하락이 아니라 기능 중단이다.
...
> ✅ **2026-08-11 구현 적용** — 인앱은 `tryFire`에서 백스톱을 무시하고 정독 85% 도달만 트리거로 쓴다.
> 비인앱 타이밍은 1ms도 바뀌지 않았다(인앱 여부를 ref로 들고 있어 타이머 effect가 재실행되지 않는다).
