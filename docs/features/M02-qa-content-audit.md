...
> ⚠️ **실측 2026-09-16** — 이 문서가 가리키는 구현 경로 **1개가 더 이상 존재하지 않는다** (그중 1개는 `agents/` 하위 — 2026-09 네이버 카페 폐기·R4 정리로 삭제됨).
...
> R4 잔여 자동화 최종 정리에서 제거했다. 아래는 **당시 운영 기록**이며 지금 동작을 설명하지 않는다.
