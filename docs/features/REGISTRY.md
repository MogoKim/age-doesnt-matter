...
|----|--------|----------|-------------|---------|---------|------------|------|------|---------|
| A01 | 카페 크롤러 파이프라인 | `agents/cafe/crawler.ts` + `psych-analyzer.ts` + `trend-analyzer.ts` + `daily-brief.ts` + `content-curator.ts` + `run-pipeline.ts` | launchd 08:30·11:30·15:30·21:30 KST (crawl-only/full/crawl-curate×2) | LOCAL_ONLY | INFRA | — | [A01](A01-cafe-crawler.md) | ACTIVE | 2026-05-14 |
