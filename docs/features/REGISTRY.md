...
> - **자동화(A·M 계열)는 대부분 정지 상태다.** GitHub Actions 26개 중 21개 `disabled_manually`(2026-08-24 이후 미실행), launchd 16개는 DB 인증 실패로 매 슬롯 실패, production `/api/health` 503.
...
> - 개별 행의 실제 상태는 **실측 필요**: `gh workflow list --all`, `launchctl list | grep -iE 'unao|unaeo'`, `agents/core/constitution.yaml` `automation_status`(현재 `PAUSED`).
