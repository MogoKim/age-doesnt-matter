...
|----|--------|----------|--------|---------|------------|------|------|---------|
| F01 | 회원가입 유도 배너 | `src/components/common/SignupPromptBanner.tsx` | 🔴 2026-09-22 **마운트 해제(비활성)** — `MainGroupClientOnly`에서 제거. 컴포넌트·단위테스트는 보존(복구는 import 1줄+렌더 1줄) | RELATION | ALL | [F01](F01-signup-prompt.md) | ARCHIVED | 2026-09-22 |
...
| F03 | 홈 페이지 | `src/components/features/home/` (21개 컴포넌트) | 홈 방문 · 2026-09-22 `SignupCard` 제거(데스크탑 하단 광고 앵커는 `HomeFaqSection`으로 이동, 광고 슬롯 불변) | RELATION | P1·P2·P3 | [F03](F03-home-page.md) | ACTIVE | 2026-09-22 |
...
| F13 | 게시글 하단 CTA | `src/components/features/community/PostCTA.tsx` | 게시글 상세(매거진 미적용) 열람 시 표시 — 🔴 2026-09-22 **비회원 가입 CTA 제거**(비회원은 렌더 없음·`cta_type='signup'` 계측 중단) / 로그인 안드로이드→Play스토어 / iOS는 `NEXT_PUBLIC_PWA_INSTALL_ENABLED` 연동 | RELATION | ALL | [F13](F13-post-cta.md) | ACTIVE | 2026-09-22 |
