...
| D-06 | 문서 | `docs/operations/soransoran-*.md` 3 + `m3-new-brand-readiness.md` + `cron-job-dispatcher-ops.md:176-210` | 소란소란 문서가 우나어 origin/main에 존재. `soransoran-d7-dday-milestone.md:781` "우나어를 지금 고치는 작업 금지"가 우나어 세션 지시로 읽힐 수 있음. 열린 PR #394도 같은 계열 | `git grep`, 에이전트 A | P1 | R5  — 2026-09-06 후속: 소란소란 3문서+m3 문서 우나어 main에서 삭제(PR-D3). 원본은 소란소란 repo `docs/archive/unao-origin-2026-08/` 커밋 `580ca358`에 보존, 우나어 AS-IS 사실은 `2026-08-21-unao-as-is-scorecard.md`로 발췌 |
...
> 실측 기록으로 보존하며, 현재 판정은 `docs/operations/UNAO_RESCUE_STATUS.md` 와 PR #434 를 따른다.
