...
>
> 🔴 **이 문서의 REVIEW 650건 판정은 [2차 분류 문서](./2026-09-11-review-tier2.md)가 대체한다.**
> 아래 REVIEW 관련 서술은 **1차(거친) 판정의 역사 기록**이며, 현재 운영 기준이 아니다.
> 2차 분류는 보호축(`protectedFromHideDelete` 32) · 조치축(`recommendedAction`) · 상태축(`currentlyGoogleNoindex` 180)을 분리했다.
>
...
> ⚠️ **당시값이다.** 2026-09-14 처분 후 현재는 `PUBLISHED` **219** · 전체 Post **3,968** 이다.
...
| **HIDE** | **60** | 비공개 권고(삭제 아님). 🔴 **"색인면에서만 내린다"는 틀렸다** — HIDE 는 사이트 공개면과 **네이버를 포함한 전 채널**에서 내린다. 2차 분류는 이 권고를 **철회**하고 HIDE 를 0건으로 뒀다 |
...
| 5 | 본문 0자 | ~~HIDE~~ | 🔴 2차 분류에서 철회 — HIDE 는 별도 고위험 결정이다 |
| 6 | **실제 SEO 노출면**(title/description)에 금지 표현 | **REVIEW** | **삭제가 아니라 문구 정정** 대상이다 |
| 7 | 동일 제목 중복 | **REVIEW** | 정본 1건을 고르는 것은 사람 판단이다 |
| 8 | 본문 300자 미만 **AND** 댓글 0 **AND** 게스트공감 0 | ~~HIDE~~ | 🔴 2차 분류에서 철회. 대상 대부분이 **이미 Google noindex** 였고(151/152), HIDE 는 네이버 공개면까지 내리는 별개 조치다 |
...
🔴 **이 절의 결론은 철회됐다.** 아래는 오판이었다.

> ~~62건 중 55건이 채용 공고의 **외부 원문 공식 직함**이라 브랜드 카피 위반과 성격이 다르다~~

2차 분류에서 `title`/`description` 으로 분해하니 **반대**였다:
JOB 55건 중 **description 에만 금지어 45건**, 그 description 이 `seoDescription` 필드
(= **우나어가 직접 쓴 카피**) 유래인 것이 **54/55**. 원문 title 직함은 10건뿐이다.
REWRITE 60건 전체로도 **BRAND_COPY 관여 59 / SOURCE_TITLE 전용 0** 이다.
→ "외부 원문이라 손댈 수 없다"는 근거는 **성립하지 않는다.** 상세: [2차 분류](./2026-09-11-review-tier2.md) §3-3
