...
## 14. 매거진 상세 추가 (2026-09-22 4차)

§13 에서 보류했던 매거진 상세에 카드를 넣었다. slot 은 **`magazine`** 으로 분리해
커뮤니티(`postdetail`)와 성과를 나눠 본다.

**배치** — `MagazineExploreLinks` 바로 **앞**:
...
🆕 SoranSoranPostCard (slot=magazine)
  ↓
MagazineExploreLinks   ← 우나어 내부 탐색 동선. 소란소란을 먼저 제안한 뒤에 둔다
  ↓
함께 읽어보세요(관련글)
  ↓
하단 애드센스
...
🔴 **광고 인접 불변**: 인아티클(위)과 하단 띠배너(관련글 뒤) 어느 쪽에도 끼어들지 않았다.
`ADSENSE.IN_ARTICLE` · `ADSENSE.POST_BOTTOM_BANNER` 슬롯 자체는 무변경이다.

`MagazineExploreLinks` 는 주석에 "검색 유입자가 우나어를 이해하고 이동하도록"이라 적힌
**우나어 내부 유도** 컴포넌트다. BRIDGE 와 방향이 반대라 **소란소란 카드를 그 앞에 둬 우선순위를 줬다**.
제거는 하지 않았다(지시 범위 밖).
