...
> 최근 갱신 **2026-09-10** — 병렬 E **완료**(production 실측) · PR #455 merge · PR #454 HOLD 해제
> 현재 main `18792f44` · production `2026.09.10-18792f4`
...
| 문서 | R0 정본화 (#433), 오도 문서 정리 (#414~#416), R5 정본화 (#443), 상태판 갱신 (#441·#445·#453) · **운영 정본 신설 (#456)** · **R7 발행 리듬 확정 (#457)** |
...
| 콘텐츠 오염 처분 | ✅ | 🔸 도구만 | ❌ | PR #454 — **HOLD 해제 · production write 0 · 실행 지시 대기** |
| 목록 SSR 크롤 경로(**병렬 E**) | ✅ | ✅ | ✅ **완료** | production 실측 2026-09-10: 6경로 각 **고유 상세 링크 12건** · `page=1`·`page=2` 겹침 0 · #418 140회 0 (PR #458 → `18792f44`) |
| **R7 콘텐츠 운영 체계** | ✅ | ✅ **구현 완료** | ❌ **대기** | PR #455 **merge 완료**(`a9835e0e`). 발행 리듬(주 3건 · 2주 연속) 운영 검증은 아직 |
...
| 구분 | 항목 |
|---|---|
| **완료** | R0 싱크 고정 · R1 로그인 소생 · R4 자동화 단순화 · R5 문서 단순화 · Next 16/React 19 전환 · 보안 업데이트(critical 0) · ops 타입 0 · **병렬 E 목록 SSR 복구**(2026-09-10 production 검증) |
| **부분 완료** | R2 네이버 신뢰(기술 PASS / 색인 0) · R3 콘텐츠 오염(격리 완료, 처분 미실행) · R6 코드·연동(패키지 의존성 감사 미착수) |
| **실행 대기** | **PR #454 네이버 카페 데이터 영구 폐기** — **HOLD 해제(2026-09-10) · production write 0 · 실행 지시 대기**. 도구·테스트·문서·dry-run 2회 완료. **그 브랜치 검증값은 test 1,176건**(신규 63건 포함)이며 `origin/main` 값이 아니다 |
| **미착수** | R8 커뮤니티 회복(관찰만) · 브랜드 금지어 63 URL · Prisma schema ↔ DB FK 불일치 정정 |
| **운영 검증 대기** | **R7 콘텐츠 운영 체계** — PR #455 **merge 완료**, 발행 리듬 2주 관찰 전 |
...
| 축 | 상태 |
|---|---|
| 기술 수집 가능 | ✅ **859 URL** 전수 200 · canonical 자기참조 100% · 일반 robots noindex 0 · X-Robots-Tag 0 · Yeti 200 · 404 정상 (2026-09-10) |
...
| PR | 상태 |
|---|---|
| **#454** 네이버 카페 데이터 폐기 도구 | **HOLD 해제 · production write 0 · 실행 지시 대기** (OPEN) |
...
[A] 네이버 회복 축   ※ 목록 SSR 복구는 여기 넣지 않는다 — 아래 [E] 하나로만 관리한다
 ...
 A-3 콘텐츠 처분 실행(#454, 실행대기) ┼─→ A-4 수집 재개 관측 → (외부) 네이버 색인
...
| 순위 | 배치 | 성격 |
|---|---|---|
| **주력 1** | **R7 콘텐츠 운영 체계** | 발행 화면은 merge 됐다(`a9835e0e`). 남은 것은 **주 3건 · 2주 연속** 운영 검증이다 |
| **주력 2** | **R8 회원 소생** | R7 산출물이 있어야 전환을 측정할 의미가 생긴다 |
| ~~독립 병렬~~ | ~~E 네이버 SSR 복구~~ | **완료 2026-09-10** — production 검증 통과 |
| **실행 대기** | **PR #454 네이버 데이터 폐기** | **HOLD 해제(2026-09-10) · production write 0 · 실행 지시 대기** |
...
| 현재 | 구현 **완료** — PR #455 **merge**(`a9835e0e`, 2026-09-10). 운영 검증(주 3건 · 2주 연속)은 **대기** |
...
| 위험 | **높음 · 불가역** |
| 현재 | **HOLD 해제(2026-09-10) · production write 0 · 실행 지시 대기** |
...
|---|---|---|---|
| **1** | PR #454 폐기 **실행 지시** | 불가역 · 14만 건 · 사용자 흔적 영향. **HOLD 는 2026-09-10 해제됨** | 실행 배치 |
...
| `docs/operations/2026-09-10-naver-cafe-purge-facts.md` · `-runbook.md` | 폐기 실측·절차 (**실행 대기** — HOLD 해제됨, 증빙으로 보존) |
