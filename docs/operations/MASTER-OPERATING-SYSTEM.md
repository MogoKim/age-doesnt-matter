...
| 완료 조건 | ① 발행 화면이 production 에서 실제로 동작 ② 발행된 글이 목록·상세·sitemap 에 정상 반영 ③ **주 N건 발행 리듬이 2주 연속 지켜짐** — ⚠️ **N 은 아직 정해지지 않았다**(창업자 결정 §11-7). N 이 정해지기 전에는 이 조건을 판정할 수 없다 ④ **발행 주체 3층이 아래 표대로 분리돼 데이터에서 확인된다** |
...
| **7** | **R7 콘텐츠 발행 방향·리듬** | 무엇을 얼마나 자주 쓸지는 창업자 콘텐츠 판단 | 주력 1 완료 조건 ③ |
