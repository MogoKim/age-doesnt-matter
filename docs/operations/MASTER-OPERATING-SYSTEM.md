...
| **미착수** | R8 커뮤니티 회복(관찰만) · 목록 SSR 크롤 경로 · 브랜드 금지어 63 URL · Prisma schema ↔ DB FK 불일치 정정 |
...
| 완료 조건 | ① 발행 화면이 production 에서 실제로 동작 ② 발행된 글이 목록·상세·sitemap 에 정상 반영 ③ **주 N건 발행 리듬이 2주 연속 지켜짐** ④ 발행 주체가 창업자임이 화면·데이터에 명확 |
| 🚫 금지 | **가짜 댓글·좋아요·회원 행세 자동화를 다시 만들지 않는다.** 발행은 사람이 한다 |
| 위험 | 중간. 콘텐츠 정책과 직결되므로 창업자 방향 확인 필요 |

### 배치 7 — R8 회원 소생 측정 (배치 6 이후)

| | |
|---|---|
| 범위 | 가입 전환 · 첫 글/첫 댓글 전환 · 답글 순환 · D1/D7 재방문을 **측정**한다 |
| 왜 | 지금은 "7일 신규가입 0 · 실회원 댓글 0" 이라는 결과만 있고 **어디서 끊기는지**를 모른다 |
| 선행 | **배치 6.** 읽을 것이 없는 상태에서 전환을 측정하면 분모가 의미 없다 |
| 완료 조건 | ① 실회원 신규가입이 7일 기준 **0 이 아님** ② 실회원 첫 댓글 전환이 측정되고 7일 기준 **0 이 아님** ③ 답글 순환(글→댓글→답글)이 실회원 사이에서 관측됨 ④ D1/D7 재방문율이 시계열로 기록됨 |
| 🚫 금지 | **가짜 댓글·좋아요·회원 행세 자동화 금지.** 지표를 만들기 위해 봇을 다시 켜지 않는다. 숫자가 0이면 0으로 기록한다 |
| 위험 | 낮음(측정). 단, "지표를 올리려 봇을 켜는" 유혹이 가장 큰 구간이다 |

### 배치 E — 네이버 SSR 복구 (독립 병렬)

배치 1과 같은 작업이며, **R7·R8 과 서로 기다리지 않는다.**
콘텐츠 공급(R7)과 검색 유입(E)은 다른 축이라 병렬로 간다.
...
| **7** | **R7 콘텐츠 발행 방향·리듬** | 무엇을 얼마나 자주 쓸지는 창업자 콘텐츠 판단 | 배치 6 완료 조건 ③ |
