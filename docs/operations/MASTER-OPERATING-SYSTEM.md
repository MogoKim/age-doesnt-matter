...
| 완료 조건 | ① 발행 화면이 production 에서 실제로 동작 ② 발행된 글이 목록·상세·sitemap 에 정상 반영 ③ **주 N건 발행 리듬이 2주 연속 지켜짐** — ⚠️ **N 은 아직 정해지지 않았다**(창업자 결정 §11-7). N 이 정해지기 전에는 이 조건을 판정할 수 없다 ④ 발행 주체가 창업자임이 화면·데이터에 명확 |
