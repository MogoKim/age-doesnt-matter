...
| **R4** | 사용하지 않는 workflow · runner · registry | **B-3 완료(2026-09-09).** 크론 키 **19개** 중 linked **14** · orphaned **5**(전부 dispatchOnly, **localOnly 0**). GHA **11개** 중 활성 4개(ci · lighthouse · quarantine-check · agents-moderation). 저장소 launchd plist 는 `unao-prod-sync` · `opsboard` 둘만 남는다 |
...
| **R4 B-3** | 외부 카페 · Google Sheet 공급망 | **REMOVE 완료(2026-09-09).** 핸들러 17키 · workflow 5개 · `agents/community/**` 전체 · 카페 크롤·큐레이션·브리프·매거진·세션 모듈 · 저장소 launchd plist 14개 제거. **persona SSoT(`curator-personas`·`curator-shared`·`persona-data`)와 `qa:content-audit` 가 쓰는 이미지 생성기는 보존** |
...
- **B-3 이후 새로 고아가 된 `agents/core/` 모듈 (2026-09-09, 미삭제 — 별도 판정 필요)**:
  `celebrity-race-blocklist.ts`(연예인·인종 비방 차단) · `medical-advice-blocklist.ts` · `coupang.ts` ·
  `google-api.ts` · `slug.ts`. 유일한 소비자였던 봇 큐레이터가 사라져 호출 경로가 0 이 됐다.
  **앞의 두 blocklist 는 안전 기능이라 임의로 지우지 않았다** — `coo:moderator` 로 옮길지, 함께 제거할지
  판정이 필요하다. (B-3 이전부터 고아였던 `approval-helper` · `locks` · `slack-commander` · `trending` 은 별건이다.)
