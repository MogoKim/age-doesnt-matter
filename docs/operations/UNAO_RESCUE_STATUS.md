...
> 로그인·네이버·커뮤니티 생존 지표를 최신 데이터로 판정할 수 있을 때까지 Rescue는 끝나지 않는다.
...
| R1 로그인 소생 | **PARTIAL** | health/auth no-store, 내부 DB 오류 노출 차단 | Kakao 로그인 -> session -> 온보딩 -> 글/댓글 운영 E2E PASS |
...
| 지표 | 현재 상태 | 다음 증거 |
|---|---|---|
| production health/auth | 최근 배포에서 200 | auth 실제 사용자 여정 |
...
| 4 | R1/R2/R3 생존 판정 | 최신 지표 후 | 로그인·검색·콘텐츠 gate별 PASS/PARTIAL/FAIL |
...
- 대기 시간을 추가 코드 삭제로 채우지 않는다. 로그인·네이버·실회원 기준선을 확보하는 데 쓴다.
...
| 체크포인트 | 날짜 | 기대값 | 현재 판정 |
|---|---|---|---|
| T+7 생존 1차 | 2026-09-12 | 로그인 실제 여정 정상, 저품질 신규 누적 중단, 네이버 지표 관찰 시작 | **진행 중** |
