...
상태: **Rescue 진행 중 — 로그인·GHA DB 인증 종결. 현재 핵심 작업은 R9 기반 단순화 / 레거시 제거**
...
- 회복 관찰: 신규 가입 · 실회원 글 · 실회원 댓글 · 재방문 회원을 **각각 단순 관찰**한다(§5-A).
  복합 대표지표는 만들지 않는다.
- 수단: SEO, 일자리, 매거진, 크롤러, 에이전트, 광고, 운영 대시보드. **쓰지 않는 수단은 제거한다(R9).**
...
| **R9 기반 단순화 / 레거시 제거** | **진행 중 — 현재 핵심 작업** | 죽은 코드 일부 제거(#428~#432), 에이전트 dependency closure read-only 조사 | 7개 대상의 KEEP/REMOVE 판정표 확정 후 승인된 작은 PR로 제거 |
...
| Gate 2 | workflow는 active, `qa:deploy-audit`은 PAUSED에서 실질 스킵 | **Gate 2 자체의 KEEP/REMOVE 재판정(R9).** PR #427은 merge하지 않는다 |
| 커뮤니티 회복 | **FAIL** - 네 숫자 단순 관찰 중 (§5-A) | 회복 추세. 기존 North Star 1명은 **임시 참고치** |
| 어드민 대시보드 | `northStar=4`는 재방문+작성이 아니라 7일 WAU. `DailyKpiSnapshot` 최신 행은 2026-08-23에서 정지 | **R9 대상** — 오래된 KPI·스냅샷·대시보드 KEEP/REMOVE 판정 |
...
- 어드민 `northStar=4`가 재방문+작성이 아니라 7일 WAU라는 사실은 기록으로 남긴다.
  화면 표기 정정은 R9 대상(오래된 KPI·대시보드)으로 넘긴다.
...
| 3 | PR #427 Gate 2 | **merge하지 않음** | Gate 2 자체의 KEEP/REMOVE 재판정(R9) 대기. 유지로 판정될 때만 최신 main 재검증 → 승인 → merge |
| 4 | **R9 기반 단순화 / 레거시 제거** | 대상별 read-only 판정부터 | 7개 대상의 KEEP/REMOVE 판정표 확정 → 승인된 작은 PR |
...
- 순위 3은 DB 인증 종결로 기술적 차단이 풀렸지만, **Gate 2를 유지할지부터 R9에서 판정한다.**
  유지 판정이 나기 전에는 merge하지 않는다.
- **순위 4(R9)가 현재 핵심 작업이다.** 지표 작업이 기반 단순화를 막지 않는다.
- 순위 2·4·5는 병렬 진행한다.
- 순위 6은 관찰이므로 다른 작업을 막지 않는다. R8 관찰 결과를 기다리느라 R9를 미루지 않는다.
...
- **R9 대상의 KEEP/REMOVE read-only 판정** — 판정표까지가 승인 없이 갈 수 있는 범위
...
## 9. R9 기반 단순화 / 레거시 제거 — 대상과 판정 기준

**현재 핵심 작업이다.** 전략 정본은 마스터플랜 §5 R9다.

### 대상

| # | 대상 | 현재 파악된 것 |
|---|---|---|
| 1 | C-level 및 역할극 에이전트 | 등록 41개 중 삭제 후보 27개(SUPERSEDED 4 / ORG_THEATER 10 / GROWTH_LEGACY 13), 유지 14개 |
| 2 | 사용하지 않는 workflow · runner · registry | 크론 키 79개 중 orphaned 33 · dispatchOnly 25. GHA 21개 `disabled_manually` |
| 3 | 보지 않는 Slack · 리포트 · AdminQueue 자동화 | Slack "NSM" 정의가 서로 다른 2벌 존재(`ceo/weekly-report.ts`, `cdo/kpi-collector.ts`) |
| 4 | 오래된 KPI · North Star · 스냅샷 · 대시보드 | `northStar`가 실제로는 WAU · `DailyKpiSnapshot` 2026-08-23 정지 · KPI 스냅샷 workflow `disabled_manually` |
| 5 | Figma 및 사용하지 않는 외부 연동 | figma MCP 연결 실패 상태로 방치 |
| 6 | 일회성 scripts · backfill | `scripts/`·`agents/scripts/` 잔여 |
| 7 | 미사용 API · 패키지 · 환경변수 · 문서 · 테스트 | `COOK82_*` 등 비활성 env, stale 문서 |
...
- 판정은 read-only로 먼저 한다. 참조원(runner·registry·workflow·문서·테스트)까지 묶어 판정표를 만든다.
