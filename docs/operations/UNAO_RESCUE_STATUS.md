...
| 단계 | 대상 | 현재 파악된 것 |
|---|---|---|
| **R4** | C-level 및 역할극 에이전트 | 등록 41개 중 삭제 후보 27개(SUPERSEDED 4 / ORG_THEATER 10 / GROWTH_LEGACY 13), 유지 14개 |
| **R4** | 사용하지 않는 workflow · runner · registry | 크론 키 79개 중 orphaned 33 · dispatchOnly 25. GHA 21개 `disabled_manually` |
| **R4** | 보지 않는 Slack · 리포트 · AdminQueue 자동화 | Slack "NSM" 정의가 서로 다른 2벌 존재(`ceo/weekly-report.ts`, `cdo/kpi-collector.ts`) |
