...
#### 보존 기간 · 개인정보 · 감사 가치 · 삭제 순서 제안 (2026-09-09 재측정)

수치는 pooler 경유 Prisma `count`/`findFirst` 로 **재측정해 위 표와 동일**함을 확인했다.
**합성 큐 데이터를 "역사"라는 이유만으로 영구 보존하지 않는다** — 봇이 스스로 만든 스케줄 행에는
감사 가치가 없다. 반대로 지표 시계열과 개인정보 인접 데이터는 각각 다른 이유로 다르게 다룬다.

| 순서 | 모델 | 건수 | 개인정보 | 감사 가치 | 제안 보존 기간 |
|---|---|---|---|---|---|
| **1** | `CommentWaveQueue` | 276 | 없음 (`postId`·`cafePostId`·`authorPersonaId` — 페르소나는 봇) | **없음** — 봇 댓글 발화 예약 시각만 담긴 합성 스케줄 | **즉시 삭제 가능**. 발행된 댓글 자체는 `Comment` 에 남아 감사 경로가 따로 있다 |
| **2** | `NaverBlogQueue` | 16 | 없음 | **없음** — 종료된 발행 큐(2026-05-30 최종). `magazinePostId` 로 원본 추적 가능 | **즉시 삭제 가능** |
| **3** | `ChannelDraft` | 653 | 없음 (`targetName` 은 방·밴드명) | **낮음** — 전부 `PENDING`(발송 0). 보내지 않은 초안이라 "무엇을 했는가"의 기록이 아니다 | **삭제 가능**. 초안 문구를 남기려면 표본만 export |
| **4** | `SocialPost` | 119 | 없음 | **중간** — 실제 외부 발행 이력(`FAILED=8`). 외부 플랫폼에 남은 게시물과 대조할 근거 | **12개월 보존 후 삭제**(최종 2026-05-16 → 2027-05 이후) |
| **5** | `UserPostWaveQueue` | 11 | **있음 — `authorId`(실회원 userId)** | 낮음 (합성 스케줄) | **개인정보 기준으로 판단.** 봇 데이터지만 실회원 식별자를 담는다. 탈퇴자 익명화(F-12)와 **정합이 맞는지 먼저 확인**하고, 맞지 않으면 익명화 대상에 포함하거나 삭제한다 |
| **6** | `DailyKpiSnapshot` | 61 | 없음 | **높음** — 2026-06-29~08-23 UV·PV·가입·WAU 시계열. 지우면 그 기간 지표를 복원할 수 없다 | **보존.** 삭제하지 않는다 |

**삭제 순서의 근거**: 위험이 낮고 대체 감사 경로가 확실한 것부터 지운다(1→2→3).
`SocialPost` 는 외부 대조 근거라 기간을 두고, `UserPostWaveQueue` 는 **개인정보 절차를 먼저 정한 뒤**
움직인다. `DailyKpiSnapshot` 은 지표라 아예 대상이 아니다.

⚠️ **이 배치에서도 삭제·migration 을 하지 않았다.** 위는 제안이며, 실행은 보존·복구 계획을
확정한 뒤 별도 migration 배치로 한다.
