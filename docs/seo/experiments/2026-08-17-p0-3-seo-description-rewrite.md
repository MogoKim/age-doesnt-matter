...
## P0-4 관찰 결과 및 판단 — MAX_DESC_LENGTH 130 → 140 (2026-08-18)

P0-4 로깅(#384)이 만들려던 판단 근거가 **표본 10건 도달로 완성**됐다.
관찰 구간: 2026-08-17 20:44 ~ 2026-08-18 12:07 KST (merge SHA `cf9655f8` 회차만 집계).

### 거부 표본 10건 전수

```
desc=skipped 총 10건
  DESC_TOO_LONG        8건   133 · 133 · 136 · 138 · 138 · 148 · 153 · 159
  ENTITY_NOT_IN_SOURCE 1건   len=115
  NUMBER_NOT_IN_SOURCE 1건   len=127
...
```
근거 ① 거부의 절반(5/10)이 131~140 — 상한 바로 위에서 잘렸다.
       preview 검수 결과 내용 품질은 정상(길이만 초과)이었고, 거부되면
       기존 원문 발췌가 그대로 남아 P0-3의 목적과 정반대 결과가 된다.
근거 ② 160자 초과는 0건 — 모델이 장황하게 쓰는 문제가 아니라
       분포의 오른쪽 꼬리가 상한에 걸리는 문제다. 프롬프트 압축은 불필요.
근거 ③ 141~160이 3건 있으므로 160까지 열지 않는다.
       이번 완화는 140까지만이며, 그 구간은 스니펫 절단 위험을 감수하지 않는다.

기대 효과: 거부 8건 중 5건(62.5%)이 통과 구간으로 들어온다.
           desc 실질 성공률(applied / 생성시도)이 63% → 약 81% 수준으로 개선 추정.
```

### 이번에 하지 않는 것 (보류 목록)

```
✗ ENTITY/NUMBER 표기 변환 정규화 — 오탐 2건(시엄니↔시어머니 · 3천↔3000)은
  같은 계열이지만 표본 2건으로는 패턴 확정 부족. 3번째 사례 시 별도 태스크 승격
✗ 프롬프트 수정 — 160+ 표본 0건이라 압축 강화 근거 없음
✗ SHEET 의료광고 gate — 별도 설계 + 30일 dry-run 선행 필요 (P1, 별도 태스크)
✗ SEO rescue 2차 — 1차 50건 색인 반응(3~7일) 관찰 완료 후
✗ MAX 140 초과 완화 — 141~160 표본이 품질 문제일 가능성이 배제되지 않았다
```

### 변경 내역

```
agents/cafe/title-rewrite-validate.ts   MAX_DESC_LENGTH 130 → 140 (단 한 곳)
테스트                                   경계값 131→141 · [70,130]→[70,140] ·
                                        TOO_LONG 데이터 138자→141자 정합화
다른 판정(COPIED/SAME_AS_TITLE/ENTITY/NUMBER)·저장 조건·title 독립성  무변경
```

### merge 후 관찰 항목

```
① 131~140 구간 desc가 실제로 applied 되는지 (len=131~140 applied 로그 첫 등장)
② desc 실질 성공률 변화 (기존 63% 대비)
③ 141+ 거부가 계속 로깅되는지 (P0-4 관찰 지속)
④ 네이버 스니펫에서 131~140자 desc가 잘리는 비율 (수동 확인, 1주 후)
```
