...
test.describe('최근 수정사항 검증', () => {
  ...
  // 임시저장 목록이 표시된 경우 "새로 작성하기"를 눌러 해제
  ...
  test('사는이야기 글쓰기 — 타이틀 + 카테고리', async ({ page }) => {
    ...
    await expect(title).toContainText('사는 이야기 글쓰기', { timeout: 10000 })
    console.log('[PASS] 사는 이야기 글쓰기 타이틀 확인')
  ...
  test('2막준비 글쓰기 — 타이틀 + 카테고리 (은퇴준비 등)', async ({ page }) => {
    ...
    // 제목 확인
    const title = page.locator('span.text-body.font-bold.text-foreground')
    await expect(title).toContainText('2막 준비 글쓰기', { timeout: 10000 })
    console.log('[PASS] 2막 준비 글쓰기 타이틀 확인')
