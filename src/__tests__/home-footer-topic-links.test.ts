...
describe('보존 — 광고·섹션 순서', () => {
  ...
  it('🔴 기존 섹션 순서가 그대로다 — 새 섹션은 일자리와 FAQ **사이**에만 들어간다', () => {
    const order = ['HeroSlider', 'FirstGreetingWidget', 'HotContentSections', 'MagazineWrapper', 'JobWrapper', 'TopicGuideSection', 'HomeFaqSection', 'SignupCard']
  ...
  it('🔴 광고 인접 관계가 유지된다 — 매거진 다음이 쿠팡2, SignupCard 다음이 데스크탑 하단', () => {
    ...
    const signup = HOME.indexOf('<SignupCard')
    ...
    // SignupCard 다음이 데스크탑 하단 광고다
    expect(HOME.indexOf('ADSENSE.DESKTOP_BOTTOM')).toBeGreaterThan(signup)
  })
})
