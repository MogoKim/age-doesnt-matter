...
describe('analyzePost — 화자 단서·세계관 위반 (배정 불가 + Haiku 표본)', () => {
  it('남성 자기발화(와이프/집사람)는 MALE_SELF', () => {
    expect(post('와이프가 요즘 말이 없어요', '집사람이랑 대화가 안 됩니다').worldviewViolation).toBe('MALE_SELF')
  })
  it("타인 인용('친구 와이프'·'와이프분' 존칭)은 MALE_SELF 아님 — 명시적 남성 1인칭만", () => {
    expect(post('친구 와이프 얘기 들었는데요').worldviewViolation).toBeNull()
