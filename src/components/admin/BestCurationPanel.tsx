...
import type { BestCurationAdminView, CurationOverrideView } from '@/lib/queries/admin/admin.home-curation'
...
export default function BestCurationPanel({ initial, previewHot, previewFame }: Props) {
  ...
  const [selectedDuration, setSelectedDuration] = useState<DurationPreset>('MANUAL')
  ...
  const reorder = (pins: CurationOverrideView[], from: number, to: number) => {
    if (to < 0 || to >= pins.length) return
    const next = [...pins]
    ;[next[from], next[to]] = [next[to], next[from]]
    startTransition(async () => {
      await reorderHomeCurationPin(activeSection, next.map(p => p.postId))
      router.refresh()
    })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-xs text-zinc-400">
        고정(PIN)은 자동 인기글 위 최상단에 표시되고(공감 수 무관 강제 노출), 숨김(HIDE)은 베스트에서 제거됩니다. 검색 시에는 큐레이션이 적용되지 않습니다.
      </p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          {sectionData.pins.length === 0 ? (
            ...
            <div className="space-y-2">
              {sectionData.pins.map((pin, idx) => (
                ...
                >
                  <span className="text-xs font-bold text-blue-500 w-4 shrink-0 text-center">{idx + 1}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-zinc-800 truncate">{pin.postTitle}</p>
                    <p className="text-xs text-zinc-400">{BOARD_LABELS[pin.postBoardType] ?? pin.postBoardType}</p>
                  </div>
                  <div className="flex items-center gap-0.5 shrink-0">
                    <button
                      onClick={() => reorder(sectionData.pins, idx, idx - 1)}
        ...
        <div>
          <h3 className="text-sm font-semibold text-zinc-700 mb-3">
            🚫 베스트에서 숨김 <span className="font-normal text-zinc-400">({sectionData.hides.length}개)</span>
          </h3>
      ...
      <details className="border border-zinc-200 rounded-xl">
        <summary className="px-4 py-3 text-sm font-semibold text-zinc-600 cursor-pointer select-none">
          현재 베스트 미리보기 — {SECTION_LABELS[activeSection]} ({previewPosts.length}개)
        </summary>
        <ul className="px-4 pb-4 mt-2 space-y-1">
          ...
          {previewPosts.length === 0 && <li className="text-xs text-zinc-400 italic">표시할 게시글 없음</li>}
        </ul>
      </details>
