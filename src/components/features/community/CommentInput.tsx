  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-end gap-2 p-4 bg-card border border-border rounded-2xl mt-2 shadow-sm max-md:sticky max-md:bottom-0 max-md:rounded-none max-md:border-x-0 max-md:border-b-0 max-md:shadow-[0_-2px_10px_rgba(0,0,0,0.08)]">
        <textarea
          className="flex-1 min-h-[52px] px-4 py-2.5 border border-border rounded-xl text-sm text-foreground bg-background resize-none outline-none transition-colors focus:border-primary focus:shadow-[0_0_0_3px_rgba(255,111,97,0.1)] placeholder:text-muted-foreground"
        ...
        <div className="flex gap-1.5">
          {onCancel && (
            <button
              className="min-h-[52px] min-w-[52px] px-3 py-2.5 bg-card text-muted-foreground border border-border rounded-xl text-xs font-bold cursor-pointer transition-all hover:text-foreground"
              onClick={onCancel}
              type="button"
            >
              취소
