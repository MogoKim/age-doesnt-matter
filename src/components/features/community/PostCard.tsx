...
export default function PostCard({ post, boardSlug }: PostCardProps) {
  return (
    ...
    >
      {post.category && (
        <span className="inline-flex items-center px-3 py-1 rounded-full bg-primary/10 text-primary text-[0.88rem] font-bold w-fit tracking-wide">{post.category}</span>
      )}

      <h2 className="text-base font-bold text-foreground m-0 line-clamp-2 leading-[1.5]">
        {post.isPinned && (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[0.88rem] font-bold tracking-wide mr-1 bg-foreground text-background">공지 </span>
        )}
        {post.promotionLevel === 'HOT' && (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[0.88rem] font-bold tracking-wide mr-1 bg-gradient-to-br from-[var(--gradient-hot-from)] to-[var(--gradient-hot-to)] text-white">HOT </span>
        )}
        {post.promotionLevel === 'HALL_OF_FAME' && (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[0.88rem] font-bold tracking-wide mr-1 bg-gradient-to-br from-[var(--gradient-fame-from)] to-[var(--gradient-fame-to)] text-white">FAME </span>
        )}
        {post.title}
      </h2>

      <p className="text-[0.88rem] text-muted-foreground m-0 line-clamp-2 leading-relaxed">{post.preview}</p>

      <div className="flex items-center gap-1.5 text-[0.88rem] text-muted-foreground pt-1">
      ...
      <div className="flex items-center gap-4 text-[0.88rem] text-muted-foreground pt-1.5 border-t border-[#f0eeec] mt-0.5">
