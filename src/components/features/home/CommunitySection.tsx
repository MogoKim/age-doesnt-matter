  ...
  return (
    <section className="py-6 lg:py-8">
      <div className="flex items-center justify-between mb-4 px-4 lg:px-0">
        ...
        <Link href="/community/stories" className="text-[0.88rem] text-muted-foreground no-underline flex items-center gap-1 p-2 -m-2 min-h-[52px] min-w-[52px] hover:text-primary">
          더보기 →
        </Link>
      </div>
          ...
          <li key={post.id}>
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span className="bg-background px-2 py-0.5 rounded text-[0.88rem] text-muted-foreground font-medium">{BOARD_LABEL[post.boardType] ?? post.boardType}</span>
                <span className="text-[0.88rem] text-muted-foreground">{post.author.nickname}</span>
              </div>
