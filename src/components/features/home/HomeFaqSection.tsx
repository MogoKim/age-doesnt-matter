...
const HOME_FAQ: { q: string; a: string; display?: ReactNode }[] = [
  ...
  {
    // JSON-LD 답변은 화면(AppInstallFaqAnswer)과 같은 정책을 말해야 한다.
    // 아이폰 "홈 화면에 추가" 안내는 2026-08-06 정책으로 화면에서 제거 → schema에서도 제거.
    q: '앱을 설치해야 하나요?',
    a: '안드로이드(삼성 등)는 구글 플레이스토어에서 우나어 앱을 받을 수 있어요. 아이폰은 따로 설치하지 않으셔도 사파리에서 그대로 모든 기능을 쓸 수 있어요.',
