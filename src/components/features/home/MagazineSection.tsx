  ...
  return (
    <section className="py-4 border-b-4 border-background lg:py-8 lg:border-b-0">
      <div className="flex items-center justify-between mb-4 px-4 lg:px-0">
        ...
        <HomeCardLink href="/magazine" className="text-caption text-muted-foreground no-underline flex items-center gap-1 p-2 -m-2 min-h-control min-w-control hover:text-primary-text" section="magazine" position={-1} contentId="" action="more">
          전체보기 →
        </HomeCardLink>
      </div>
      ...
      <div className="relative">
        <div className="flex gap-3 px-4 pr-12 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden [scroll-snap-type:x_mandatory] lg:grid lg:grid-cols-4 lg:gap-4 lg:px-0 lg:pr-0 lg:overflow-x-visible lg:[scroll-snap-type:none]">
          {posts.map((article, index) => (
            <HomeCardLink
              href={buildPostPath({ id: article.id, boardType: 'MAGAZINE', slug: article.slug })}
              key={article.id}
