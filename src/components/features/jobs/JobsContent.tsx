    ...
    <>
      {hasFilters && (
        <div className="flex items-center gap-2 mb-4 text-body text-muted-foreground">
          ...
          {region && (
            <span className="px-2.5 py-1 rounded-full bg-primary/10 text-primary-strong text-caption font-medium">
              {region}
            </span>
          )}
          {tags?.map((tag) => (
            <span key={tag} className="px-2.5 py-1 rounded-full bg-primary/10 text-primary-strong text-caption font-medium">
              {tag}
            </span>
          ))}
        </div>
