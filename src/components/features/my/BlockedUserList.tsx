      ...
      {users.map((user) => (
        <div key={user.id} className="flex items-center justify-between p-3 bg-background rounded-xl">
          ...
          <button
            ...
            disabled={isPending}
            className="min-h-[52px] px-4 py-2 text-[0.88rem] font-bold text-destructive border border-destructive/30 rounded-lg bg-destructive/5 transition-all hover:bg-destructive/10 disabled:opacity-50"
          >
            차단 해제
          </button>
        </div>
