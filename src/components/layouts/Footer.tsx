...
export default function Footer() {
  return (
    <footer className="mx-auto flex w-full max-w-lg flex-col items-center bg-background px-4 pb-6 lg:pb-8">
      {/* 0. 브랜드 헤더 — 로고 심볼(겹친 원 crop) + 워드마크 + 소개 */}
      <div className="flex flex-col items-center gap-1 py-5">
        <div className="flex items-center gap-1.5">
          {/* 실제 로고에서 심볼(상단 원)만 노출, 워드마크 영역은 overflow로 가림 (IdentityBanner 패턴) */}
      ...
      {/* 1. 정책 링크 */}
      <nav className="flex w-full flex-wrap justify-center gap-x-3 gap-y-1 border-t border-[#f1f1f1] py-3.5" aria-label="하단 링크">
      ...
      {/* 2. 공식 채널(SNS 아이콘) + 3. Google Play (FooterChannelLinks가 자체 구분선 섹션으로 렌더, Play는 웹 전용) */}
      <FooterChannelLinks />

      {/* 4. 글씨 크기 — 컴팩트 알약 그룹 */}
      <div className="flex w-full justify-center border-t border-[#f1f1f1] py-3.5">
