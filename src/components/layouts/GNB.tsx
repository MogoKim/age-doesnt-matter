...
const MENU_ITEMS = [
  { label: '베스트',     href: '/best' },
  { label: '갱년기톡',  href: '/community/menopause' },
    ...
    <nav className="hidden lg:flex sticky top-0 z-[100] h-16 bg-card border-b border-border items-center justify-center" aria-label="메인 네비게이션">
      <div className="flex items-center w-full max-w-[1200px] px-8 gap-4">
        ...
        <div className="relative shrink-0">
          <form className="flex items-center w-52 min-h-control pl-4 pr-0 bg-background border border-border rounded-lg text-caption text-foreground transition-colors focus-within:border-primary" onSubmit={handleSearch} role="search" aria-label="통합검색">
            <input
              className="min-w-0 flex-1 border-none bg-transparent outline-none font-[inherit] text-inherit min-h-control placeholder:text-muted-foreground"
              type="search"
              placeholder="통합검색"
              value={query}
              ...
              aria-label="통합검색"
            />
            <button type="submit" className="icon-hover flex shrink-0 items-center justify-center w-control h-control rounded-lg text-muted-foreground hover:text-primary-text" aria-label="검색">
              <IconSearch size={18} />
            </button>
          </form>
          {searchError && (
