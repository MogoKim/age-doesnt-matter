...
export default function IconMenu() {
  ...
  return (
    <nav className="sticky top-[56px] z-[99] min-h-[64px] bg-card border-b border-border flex items-center lg:hidden overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden px-0 gap-0" aria-label="주요 메뉴">
      {MENU_ITEMS.map((item) => {
        ...
        return (
          <Link
            ...
            href={item.href}
            className={cn(
              'nav-icon-hover flex flex-col items-center justify-center gap-1 flex-1 min-w-[64px] min-h-[52px] p-1 no-underline text-muted-foreground relative [-webkit-tap-highlight-color:transparent] hover:text-primary-text',
