 ...
 *
 * ## 왜 0인가 (2026-08-11 P0 보정)
 * 0.8이었을 때 Dock이 **첫 광고가 끝나기 전에** 떠서 광고를 다시 덮었다.
 * 프로덕션 실측: 겹침 0/41 → **3/41**, 최악은 광고 299px 중 69px(23%) 가림 —
 * PR #333에서 고쳤던 것과 같은 수치다.
 *
 * 0으로 두면 "섹션이 실제로 화면에 들어왔을 때"가 된다.
 * 측정한 4개 글 전부 그 시점이 **첫 광고가 화면을 빠져나간 뒤**였다(각 +127px):
 *   글      첫 광고 끝 통과 → 마진 0일 때 Dock
 *   post1   1025 → 1152
 *   post2    651 →  778
 *   post3    537 →  664
 *   post4    722 →  849   (기존 기준 2086 대비 1237px 빠름)
 *
 * 광고를 셀렉터로 피하지 않는다 — provider 마크업이 바뀌면 조용히 깨진다.
 * 대신 "댓글 섹션이 화면에 들어왔다"는 **콘텐츠 기준**만으로 같은 결과를 얻는다.
 */
export const NEAR_VIEWPORT_RATIO = 0
...
export function resolveDockVisibility(input: DockVisibilityInput): DockVisibilityResult {
  ...
  // 댓글 **섹션 시작이 화면에 들어오면** 띄운다. 목록을 다 지나칠 때까지 기다리지 않는다.
  //   마진 0이라 "미리 당겨 띄우지" 않는다 — 당기면 첫 광고가 아직 화면에 걸려 있어 덮게 된다.
