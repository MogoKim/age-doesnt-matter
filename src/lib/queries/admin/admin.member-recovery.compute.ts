...
/**
 * 방문자별 이벤트 **발생 구간**(최초·최종). `_min`/`_max` 집계라 원본 행을 끌어오지 않는다 —
 * `page_view` 처럼 행이 많은 이벤트도 DB 에서 접어서 가져온다.
 *
 * 구간이 필요한 이유: 퍼널은 **순서가 있는** 전환이다. 단순 집합 교집합은
 * "배너를 본 뒤 눌렀다"와 "누른 뒤 배너를 봤다"를 구분하지 못한다(`advancedAfter` 참조).
 */
