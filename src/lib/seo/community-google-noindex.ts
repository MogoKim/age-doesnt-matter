/**
 * Google 전용 저품질 커뮤니티 글 색인 제외 판단 (PR-B1, 순수 함수 — DB/서버 의존 없음).
 *
 ...
 *
 * 1차는 "가장 명확한 저품질"만 자른다. 반응·본문·메타 중 하나라도 있으면 보호한다.
 * MENOPAUSE(갱년기톡)는 핵심 성장축이라 1차에서 전면 보호한다.
 * 신규 글은 14일간 유예한다 — 짧아도 커뮤니티 반응이 붙을 시간을 준다.
 *
 * ※ greeting/이벤트 글의 기존 `robots {index:false, follow:false}` 정책이 항상 우선한다(호출부에서 처리).
 */
import { stripHtmlTags } from '@/lib/sanitize'

/** googlebot noindex를 적용할 수 있는 보드 (MENOPAUSE·MAGAZINE·JOB 등은 대상 아님) */
const TARGET_BOARD_TYPES = ['STORY', 'LIFE2', 'HUMOR'] as const
...
/**
 * 구글 전용 noindex 대상인지 판단한다. 아래 조건을 **모두** 만족할 때만 true.
 *   1) boardType이 STORY/LIFE2/HUMOR
 *   2) 본문 텍스트 300자 미만
 *   3) 댓글 0개
...
export function shouldGoogleNoindexCommunityPost(
  ...
  // 1) 대상 보드만 (MENOPAUSE·MAGAZINE·JOB·기타 전부 보호)
