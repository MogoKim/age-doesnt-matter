...
// ── Slack 클라이언트 (stop, meeting에서 사용) ──
const SLACK_BOT_TOKEN = process.env.SLACK_BOT_TOKEN ?? ''
const slack = SLACK_BOT_TOKEN ? new WebClient(SLACK_BOT_TOKEN) : null

async function sendToChannel(channelEnvKey: string, text: string): Promise<void> {
  ...
  try {
    ...
    console.error(`[Slack] ${channelEnvKey} 전송 실패:`, err)
  }
}
...
async function handleApprove(text: string): Promise<SlackCommandResult> {
  const id = text.trim()
  if (!id) {
    return { response_type: 'ephemeral', text: '사용법: /una-approve [ID]' }
  }
  return {
    response_type: 'ephemeral',
...
async function handleReject(text: string): Promise<SlackCommandResult> {
  const id = text.trim()
  if (!id) {
    return { response_type: 'ephemeral', text: '사용법: /una-reject [ID]' }
  }
  return {
    response_type: 'ephemeral',
